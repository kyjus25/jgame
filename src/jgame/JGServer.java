...
class JGCreateRequest {
    ...
    public JGCreateRequest(String type, String posX, String posY, String uuid) {
        this.type.set(type);
        this.posX.set(posX);
        this.posY.set(posY);
        this.uuid.set(uuid);
...
//    }
    ...
    public void sendLastKnownPositions(String nick) {
        ...
        if (player == null) {
            JGCreateRequest newPlayer = new JGCreateRequest(JGame.networkManager.networkSprite.get(), "EMPTY", "EMPTY", nick);
    ...
    public void sendAll(String s, String sender) {
        users.forEach(u -> {
            if (!u.nick.get().equals(sender) && !u.nick.get().equals(users.get(0).nick.get())) {
                u.submit(s);
            }
        });
        JGame.networkManager.packetStream.add(s);
    }
    ...
    public void sendHost(String s) {
        JGame.networkManager.packetStream.add("UPDATE " + s);
    }
    ...
    public JGCreateRequest getJGCreateRequest(String sender, String packet) {
        ...
        return new JGCreateRequest(type, posX, posY, uuid);
    }
            ...
            if (queue.size() > 0) {
                ...
                copy.forEach(item -> {
                    ...
                    if (i.savePos.get() && i.uuid.get() != null) {
                        ...
                        sendAll("UPDATE " + i.type.get() + " " + i.posX.get() + " " + i.posY.get() + " " + i.uuid.get());
                    } else {
                        if (i.type.get() != null && i.type.get().contains("EVENT")) {
                            if (i.type.get().contains("DELETE")) {

                                JGCreateRequest entry = lastKnownPos.get(i.type.get().split("[ ]")[1]);
                                if (entry != null) {
